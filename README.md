...
当前项目已完成 M1、M1.5、M2、M3 基础版、M3.1 收尾版和 M4.1 搜索推荐基础版：Feed、搜索、笔记详情、用户主页和后台列表已切换到 Prisma 查询；跨端 API contract、i18n 字典、权限边界、Docker Compose、migration 审查和 CI 已落地；登录、注册、发布、预签名上传和 `/publish`、`/admin` 权限保护已接通；点赞、收藏、一级评论、二级回复、一级评论分页、关注、通知写入、通知中心页面/API、未读数、标记已读、互动限流、目标冷却、重复评论保护、`/api/v1` 互动写接口、评论删除/举报、后台隐藏/驳回、评论敏感词拦截、用户屏蔽和不感兴趣过滤已接通。M4.1 已让 Feed 按推荐分排序，搜索支持关键词 + pgvector 语义召回兜底、命中原因和 `/api/v1/feed`、`/api/v1/search` cursor 分页。下一步继续 M4.2，重点做搜索建议、热搜、历史搜索和推荐候选缓存。

## Project Shape

- `src/app/(site)` C 端页面：Feed、搜索、发布、笔记详情、用户主页、通知中心
- `src/app/admin` 管理后台：数据看板、笔记、举报、用户
- `src/app/api/v1` 跨端 API：Feed、搜索、图片预签名上传、点赞、收藏、评论、评论治理、关注、屏蔽、不感兴趣、通知列表和已读
