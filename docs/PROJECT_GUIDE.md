...
| `/search` | `src/app/(site)/search/page.tsx` | 搜索结果页，展示关键词/语义召回后的命中原因、搜索建议、热搜和历史 |
...
| `/admin/safety` | `src/app/admin/safety/page.tsx` | 内容安全词库只读巡检、单段文本检测和逐行批量检测 |
...
| `/api/v1/search/discovery` | `src/app/api/v1/search/discovery/route.ts` | 跨端搜索发现 API，返回建议、热搜、历史和分类统计 |
...
- 敏感词：发布笔记和评论写入前会经过 `src/lib/content-safety.ts` 的基础敏感词检查；命中时返回表单/API 错误，不会写入内容。评论和笔记词库可分别通过 `COMMENT_SENSITIVE_TERMS`、`NOTE_SENSITIVE_TERMS` 配置。
...
当前核心读链路已有 MVP 级 Redis 缓存：Feed 推荐候选池、搜索热词、个人搜索历史和趋势话题已接入 Redis。详情页浏览量仍是同步数据库递增，这些实现仍适合 MVP，不适合大流量公开访问；后续优化时应优先处理浏览量聚合、搜索索引、缓存指标和更细的失效策略。

跨端 API 基础约定已经在 `src/lib/api-contract.ts` 中定义。当前 `/api/v1/feed`、`/api/v1/search`、`/api/v1/search/discovery`、`/api/v1/uploads`、`/api/v1/notifications` 和 `/api/v1` 互动写接口返回统一 `ok/version/data` envelope。Feed/Search 已支持 `cursor` 和 `limit`，通知列表使用通知 id 做 cursor 分页；后续移动端接入时应继续沿用这套 `pageInfo` 结构。
...
- 后台数据看板读取同一个 Redis 热搜 zset 的成员数作为搜索热词指标，便于先观察搜索活跃度；真正的搜索转化率仍需要后续曝光、点击和结果页行为埋点。
...
后续推荐服务应继续把每个信号归一化到 0 到 1，并保留排序解释，方便调试和后台观察。`scoreRecommendation` 会对最终输入做 0 到 1 夹值兜底，避免语义召回或外部特征异常污染排序。Feed 推荐候选已经放入 Redis，详情页和作者主页已有基础推荐区；推荐评分已有基础单元测试，语义召回失败已有降级日志。下一步重点是补更多排序边界测试和基础监控。
...
- 内容安全页展示评论/笔记敏感词来源，支持单段命中测试和逐行批量巡检。
- 数据看板展示用户总数、已发布笔记、待审举报、互动总数和搜索热词数量；Redis 不可用时搜索热词指标降级为 0 并标注连接状态。
